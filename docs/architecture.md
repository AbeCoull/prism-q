...
### [2026-04-22] GPU BTS crossover and stabilizer benchmark split
- **Context**: GPU BTS sampling re-uploaded sparse parity data on every call.
  Stabilizer GPU measurement also allocated one scratch word per measurement.
  Benchmark groups mixed kernel timing with probability readback.
- **Options considered**: keep the `65_536` BTS default with cache reuse only,
  lower the default to the first packed-shot win, or raise the default above the
  measured regressing buckets while keeping explicit GPU access available through
  `PRISM_GPU_BTS_MIN_SHOTS`.
- **Decision**: cache GPU BTS parity data and chunk scratch on the sampler. Cache
  stabilizer measurement pivot and outcome scratch on the tableau. Benchmark
  stabilizer throughput with direct backend timing that excludes
  `probabilities()` readback. Set `BTS_MIN_SHOTS_DEFAULT = 131_072` so default
  dispatch stays on CPU for measured regressing BTS buckets.
- **Measurement**: With the old `65_536` default, packed BTS measured 512q,
  10k shots at CPU 284.79 microseconds and GPU 208.36 microseconds. Regressing
  buckets were 512q, 100k shots at CPU 4.19 ms and GPU 11.99 ms, plus 2048q,
  100k shots at CPU 16.96 ms and GPU 46.25 ms. After raising the default,
  `gpu_bts_packed` measured 512q, 100k shots at CPU 3.98 ms and GPU 3.64 ms,
  and 2048q, 100k shots at CPU 16.12 ms and GPU 16.84 ms. Direct stabilizer
  backend timing measured Clifford depth 10 at 1000q: CPU 4.55 ms and GPU
  1.41 ms. At 5000q: CPU 38.24 ms and GPU 22.28 ms.
- **Revisit trigger**: revisit when GPU BTS gains a circuit shape aware crossover
  model, or when direct packed-shot benchmarks show a stable GPU win at and above
  the default threshold.

### [2026-04-22] Device reductions for GPU BTS
- **Context**: GPU BTS sampling copied the full shot matrix back to the host when
  callers only needed marginals or exact counts, erasing much of the large-shot
  benefit on wide measurement workloads.
- **Options considered**: keep `sample_bulk_packed` as a host API, add a packed
  shot handle with device row-popcount and hash reductions, or add streaming
  reductions without an explicit device-shot type.
- **Decision**: add `DevicePackedShots` for flat sparse GPU BTS. Route
  `CompiledSampler::sample_marginals` through device row-popcount reduction. Add
  bounded exact counts that keep the GPU result only when the compact
  `(bitstring, count)` payload is smaller than the raw packed shot matrix.
- **Measurement**: `bench_gpu` includes `gpu_bts_marginals*` groups for the public
  marginal API and `gpu_bts_device_counts*` groups for the explicit device handle
  counts path. These isolate reduced transfer reductions from raw packed-shot
  materialization.
- **Revisit trigger**: revisit when GPU BTS adds streaming reductions for generic
  accumulators, or when exact device counts need support beyond 8 packed
  measurement words.
