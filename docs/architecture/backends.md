...
`FxHashMap<usize, Complex64>` (the crate's multiply-xor hasher) for states with few non-zero amplitudes. O(k) memory. Entries at or below a pruning threshold on |a|² (1e-28, roundoff level) are removed after gates that can shrink or cancel amplitudes, and the kept entries are rescaled so the state keeps its norm; a run whose threshold was raised above the default reports `Approximate` with a fidelity bound derived from the dropped weight. Best for circuits whose support stays concentrated in computational-basis states at large qubit counts.
